...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createSmoke(x, y) {
        ...
        for (let i = 0; i < 15; i++) {
            ...
            particle.style.background = `rgba(156, 39, 176, ${0.3 + Math.random() * 0.4})`;
    ...
    function crashUFO(event) {
        ...
        // Create smoke effect during crash
        let smokeInterval = setInterval(() => {
            createSmoke(event.clientX + (Math.random() * 40 - 20), 
                       event.clientY + (Math.random() * 40 - 20));
        }, 100);

        // Stop smoke and hide UFO after crash
        setTimeout(() => {
            clearInterval(smokeInterval);
            ufo.classList.add('hidden');
            ufo.classList.remove('crashing');

            // Re-enter from side after a delay
            setTimeout(() => {
